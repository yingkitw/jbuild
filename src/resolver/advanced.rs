...
mod tests {
    ...
    fn test_compare_versions() {
        assert!(VersionRangeResolver::compare_versions("1.0.0", "1.0.1") < 0);
        assert!(VersionRangeResolver::compare_versions("1.1.0", "1.0.0") > 0);
        assert_eq!(VersionRangeResolver::compare_versions("1.0.0", "1.0.0"), 0);
    }
    ...
    fn test_conflict_resolver_nearest_wins() {
        ...
        let conflicts = vec![
            ("lib:1.0.0".to_string(), artifact1.clone()),
            ("lib:2.0.0".to_string(), artifact2.clone()),
        ];

        let resolved = ConflictResolver::resolve_conflicts(&conflicts);
        assert_eq!(resolved.len(), 2); // Both kept (nearest wins keeps all)
    }
    ...
    #[test]
    fn test_version_key() {
        let key1 = DependencyMediator::version_key("1.0.0");
        let key2 = DependencyMediator::version_key("2.0.0");
        assert!(key2 > key1);
    }
}
